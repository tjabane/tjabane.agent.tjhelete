...
| --- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| H1  | In progress | The current `BankApiClient` uses account-specific balance and transaction endpoints, while financial tools must cover the complete account set. | `BankApiClient.getAccounts()`, `getAccountBalance(accountId)`, and their Investec validation/mapping are implemented. Remaining: high-level application capabilities must query all returned accounts and reject incomplete balance, transaction-list, and spending-summary aggregates. The model chooses these high-level tools and their order; provider account identifiers and endpoint choreography remain internal. |
...
| M4  | Planned | The proposed catalog includes balances, budgets, goals, categorisation, and notifications, while the current services package supports posted transactions only. | Separate the target catalog from the initially enabled tools and enable a tool only when its application capability exists.                                                               |
| M5  | Planned | Ownership between the agent contracts package and concrete tools package is not completely explicit.                                                             | Keep `ToolRegistry`, `AgentTool`, and shared result contracts in the agent package; keep concrete tools and their metadata in the tools package; wire both from the API composition root. |
...
1. H1: discovery and complete aggregation of all Investec-authorised accounts.
2. H2: bounded tool-call execution.
3. H3: safe and idempotent write execution.
4. M1-M5: contract alignment and initial implementation scope.
5. L1-L4: hardening before broader rollout.
